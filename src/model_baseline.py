...
df["geo_delta"] = find_geo_delta(df["t_lat"], df["t_lon"], df["m_lat"], df["m_lon"])
print(df.head())

y = df["is_fraud"]
X = df.drop(columns=["is_fraud", "txn_id", "txn_ts"])
X_train, X_val, y_train, y_val = sklearn.model_selection.train_test_split(X, y, test_size=0.25, random_state=123, stratify=y)
